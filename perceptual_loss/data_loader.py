...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        rep_audio_mel = self.data[index]["rep_audio_mel"]
        ...
        return torch.FloatTensor(audio),torch.FloatTensor(vertice), torch.FloatTensor(template), torch.FloatTensor(one_hot), file_name, rep_audio_mel
    ...
    for r, ds, fs in os.walk(audio_path):
        for f in tqdm(fs):
            if f.endswith("wav"):
                ...
                ## guidance
                data[key]["rep_audio_mel"] = load_audio_mel(wav_path)
